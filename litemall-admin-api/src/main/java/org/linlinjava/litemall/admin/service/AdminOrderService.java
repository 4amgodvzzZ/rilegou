...
public class AdminOrderService {
    ...
    public Object list(Integer userId, String orderSn,String date, List<Short> orderStatusArray,
        ...
        List<LitemallOrder> orderList = orderService.querySelective(userId, orderSn,date, orderStatusArray, page, limit, sort, order,regionid);
        List<LitemallOrder> newOrderList = new ArrayList<>();
        if (orderList.size()>0){
            for (LitemallOrder litemallOrder:orderList){
                //订单所属代理点名称
                LitemallRegion litemallRegion = litemallRegionService.findById(litemallOrder.getRegionId());
                litemallOrder.setRegionName(litemallRegion.getName());
                newOrderList.add(litemallOrder);
            }
        }
