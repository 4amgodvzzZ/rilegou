...
//        drawTextInImgCenter(baseImage, shopName, 98);
    ...
    public String createUserOrderImage(String userName, String userPicUrl, Integer userid) {
        try {
            //创建该商品的二维码
            File file = wxMaService.getQrcodeService().createWxaCodeUnlimit("saleduserid," + userid, "pages/ucenter/saleorder/saleorder");
            FileInputStream inputStream = new FileInputStream(file);
            //将商品图片，商品名字,商城名字画到模版图中
            byte[] imageData = drawPicture(inputStream, userPicUrl, userName);
            ByteArrayInputStream inputStream2 = new ByteArrayInputStream(imageData);
            //存储分享图
            String url = storageService.store(inputStream2, imageData.length, "image/jpeg", getKeyName(userid.toString()));
