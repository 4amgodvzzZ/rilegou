...
public class WxAuthController {
    ...
    public Object loginByWeixin(@RequestBody WxLoginInfo wxLoginInfo, HttpServletRequest request) {
        ...
        Integer regionid = wxLoginInfo.getRegionid();
        ...
        //获取当前user的regionid对应的代理商位置信息
        LitemallAdmin angentInfo = new LitemallAdmin();
        LitemallRegion region = new LitemallRegion();
        ...
        if (user == null) {
            ...
            if (regionid>0){
                user.setRegionId(regionid);
                angentInfo =  litemallAdminService.querySelectiveLitemall(regionid,0);
                region = litemallRegionService.findById(regionid);
            }else {
                angentInfo =  litemallAdminService.querySelectiveLitemall(user.getRegionId(),0);
                region = litemallRegionService.findById(user.getRegionId());
            }
        ...
        result.put("tokenExpire", userToken.getExpireTime().toString());
        result.put("userInfo", userInfo);
        result.put("angentInfo", angentInfo);
        result.put("regionInfo", region);
    ...
    public Object register(@RequestBody String body, HttpServletRequest request) {
        ...
        //获取当前user的region小区位置信息
        LitemallAdmin angentInfo =  litemallAdminService.querySelectiveLitemall(user.getRegionId(),0);
        // userInfo
        UserInfo userInfo = new UserInfo();
        userInfo.setNickName(username);
        userInfo.setAvatarUrl(user.getAvatar());
    ...
    @RequestMapping("info")
    public Object getAuthInfo(@LoginUser Integer userId){
        LitemallUser user = userService.findById(userId);

        Map<Object, Object> result = new HashMap<Object, Object>();
        result.put("authInfo", user);

        if (user == null) {
            return ResponseUtil.fail(AUTH_CAPTCHA_FREQUENCY, "获取用户数据失败！");
        }
        return ResponseUtil.ok(result);
    }
