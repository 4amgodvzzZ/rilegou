...
public class WxOrderService {
     ...
     * @return 订单列表
     */
    public Object agentlist(Integer userId,Integer saleduserid, String orderdate,Integer showType,Integer page, Integer size) {
        LitemallAdmin litemallAdmin = litemallAdminService.querySelectiveLitemall(0,userId);

        List<Short> orderStatus = OrderUtil.orderStatus(showType);//代理商只需看到用户在他门店里面的所有的订单
        List<LitemallOrder> orderList = orderService.queryByOrderStatusAndRegionid(litemallAdmin.getRegionId(),saleduserid,orderdate, orderStatus);
        int count = orderService.countByOrderStatusAndRegionid(litemallAdmin.getRegionId(),saleduserid,orderdate, orderStatus);
        ...
        for (LitemallOrder order : orderList) {
            ...
            orderVo.put("orderSn", order.getOrderSn());
            orderVo.put("actualPrice", order.getActualPrice());
            orderVo.put("selfMentionSign", order.getSelfMentionSign());
            orderVo.put("orderStatusText", OrderUtil.orderStatusText(order));
            orderVo.put("handleOption", OrderUtil.build(order));
            ...
            List<LitemallOrderGoods> orderGoodsList = orderGoodsService.queryByOid(order.getId());
            List<Map<String, Object>> orderGoodsVoList = new ArrayList<>(orderGoodsList.size());
            BigDecimal agentPer = SystemConfig.getAgentMoney(); //代理点获取40%分成
            BigDecimal orderGoodsSum = new BigDecimal(0); //该订单所有商品的供货价之和
            for (LitemallOrderGoods orderGoods : orderGoodsList) {
                LitemallGoods litemallGoods = litemallGoodsService.findById(orderGoods.getGoodsId());
                orderGoodsSum = orderGoodsSum.add(litemallGoods.getProvidePrice());
                Map<String, Object> orderGoodsVo = new HashMap<>();
                orderGoodsVo.put("id", orderGoods.getId());
                orderGoodsVo.put("goodsName", orderGoods.getGoodsName());
                orderGoodsVo.put("number", orderGoods.getNumber());
            ...
            orderVo.put("goodsList", orderGoodsVoList);
            //如果是客户派送订单 派送费不计入利润中
            if (order.getSelfMentionSign()==0){
                //订单真实付款 - 订单商品供货价之和 - 派送费 * 分成占比 = 代理点获利
                orderVo.put("agentMoney", order.getActualPrice().subtract(orderGoodsSum).subtract(SystemConfig.getFreight()).multiply(agentPer).setScale(2,BigDecimal.ROUND_DOWN));
            }else {
                //订单真实付款 - 订单商品供货价之和 * 分成占比 = 代理点获利
                orderVo.put("agentMoney", order.getActualPrice().subtract(orderGoodsSum).multiply(agentPer).setScale(2,BigDecimal.ROUND_DOWN));
            }
    ...
    public Object detail(Integer userId, Integer orderId) {
        ...
        List<LitemallOrderGoods> orderGoodsList = orderGoodsService.queryByOid(order.getId());
        LitemallAdmin litemallAdmin = litemallAdminService.querySelectiveLitemall(order.getRegionId(),0);

        Map<String, Object> result = new HashMap<>();
        result.put("orderInfo", orderVo);
        result.put("orderGoods", orderGoodsList);
        result.put("agentinfo", litemallAdmin);
...
//        }
     ...
     * @return 订单详情
     */
    public Object detailshare(Integer orderId) {
        // 订单信息
        LitemallOrder order = orderService.findById(orderId);
        if (null == order) {
            return ResponseUtil.fail(ORDER_UNKNOWN, "订单不存在");
        ...
        //找出该订单下商品有哪些用户也购买了
        List<LitemallOrderGoods> allOrderGoodsList = new ArrayList<>();
        List<LitemallUser> litemallUsers = new ArrayList<>();
        for (LitemallOrderGoods ordergoods:orderGoodsList) {
            allOrderGoodsList = orderGoodsService.queryByGid(ordergoods.getGoodsId());
        }
        for (LitemallOrderGoods ordergoods:allOrderGoodsList) {
            LitemallUser litemallUser = userService.findOrderBuyUsersById(ordergoods.getUserId());
            litemallUsers.add(litemallUser);
        }
        // 利用set集合唯一性对重复购买用户去重
        Set set = new HashSet(litemallUsers);
        List newlitemallUsers = new ArrayList(set);

        Map<String, Object> result = new HashMap<>();
        result.put("orderInfo", orderVo);
        result.put("orderGoods", orderGoodsList);
        result.put("agentinfo", litemallAdmin);
        result.put("orderBuyUsers", newlitemallUsers);
    ...
    public Object submit(Integer userId, String body) {
        ...
        Integer selfMentionSign = JacksonUtil.parseInteger(body, "selfMentionSign");//自提标志 1为自提 运费为0
        Integer regionId = JacksonUtil.parseInteger(body, "regionId");//门店rengionid
        Integer parentRegionId = JacksonUtil.parseInteger(body, "parentRegionId");//地级市regionid
        ...
        // 根据订单商品总价计算运费，满足条件（例如66元）则免运费，否则需要支付运费（例如2元）；
        BigDecimal freightPrice = new BigDecimal(0.00);
        if (checkedGoodsPrice.compareTo(SystemConfig.getFreightLimit()) < 0) {
            freightPrice = SystemConfig.getFreight();
        }

        if (selfMentionSign==1){
        ...
        order.setpRegId(parentRegionId);
        order.setSelfMentionSign(selfMentionSign);
